...
[[nodiscard]] WaitResult FutexWait(const std::atomic<uint32_t>& futex, const int expected,
                                   const timespec* timeout = nullptr);
[[nodiscard]] bool FutexWake(const std::atomic<uint32_t>& futex, const int count);
