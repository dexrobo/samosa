...
H264Encoder::H264Encoder(const Params& params) : params_(params) {
  ...
  if (rv != 0 || encoder_ == nullptr) {
    SPDLOG_ERROR("WelsCreateSVCEncoder failed: {}", rv);
    encoder_ = nullptr;
    return;
  }
  ...
  ext_params.iTargetBitrate = static_cast<int>(params_.bitrate_kbps) * 1000;
  ...
  auto& layer = ext_params.sSpatialLayers[0];
  ...
  rv = encoder_->InitializeExt(&ext_params);
  if (rv != 0) {
    SPDLOG_ERROR("OpenH264 InitializeExt failed: {}", rv);
...
std::optional<H264Encoder::EncodedFrame> H264Encoder::Encode(const uint8_t* yuv_i420, uint64_t timestamp_us) {
  ...
  pic.uiTimeStamp = static_cast<long long>(timestamp_us / 1000);  // OpenH264 expects milliseconds.

  const auto w = static_cast<int>(params_.width);
  const auto h = static_cast<int>(params_.height);
  ...
  pic.pData[1] = const_cast<unsigned char*>(yuv_i420 + w * h);
  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
  pic.pData[2] = const_cast<unsigned char*>(yuv_i420 + w * h + (w / 2) * (h / 2));
  pic.iStride[0] = w;
  pic.iStride[1] = w / 2;
  pic.iStride[2] = w / 2;
  ...
  if (rv != 0) {
    SPDLOG_WARN("EncodeFrame failed: {}", rv);
    return std::nullopt;
  }
  ...
  for (int layer_idx = 0; layer_idx < info.iLayerNum; ++layer_idx) {
    const auto& layer_info = info.sLayerInfo[layer_idx];
    const uint8_t* buf = layer_info.pBsBuf;
    for (int nal_idx = 0; nal_idx < layer_info.iNalCount; ++nal_idx) {
      int nal_len = layer_info.pNalLengthInByte[nal_idx];
      // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
